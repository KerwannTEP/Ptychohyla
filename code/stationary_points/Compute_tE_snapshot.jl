...
function plot_Lagrange_points(namefile)

    # namefile = path_data*"snapshots_"*srun_data*"/time_50.0.txt"
    ...
    # Circular orbit 
    nbo = 500
    tabtx = [d_host * R_HU_in_kpc * cos(2*pi/nbo * (i-0.5)) for i=1:nbo]
    tabty = [d_host * R_HU_in_kpc * sin(2*pi/nbo * (i-0.5)) for i=1:nbo]

    p = plot(tabtx, tabty, 
                xlims=(-d_host*R_HU_in_kpc, d_host*R_HU_in_kpc),
                ylims=(-d_host*R_HU_in_kpc, d_host*R_HU_in_kpc),
                linestyle=:dash,
                color=:red,
                aspect_ratio = 1, label=:false)

    scatter!(p, [(xL4+mu_c*d_host) * R_HU_in_kpc], [yL4 * R_HU_in_kpc], 
                xlims=(-d_host*R_HU_in_kpc, d_host*R_HU_in_kpc),
                ylims=(-d_host*R_HU_in_kpc, d_host*R_HU_in_kpc),
                aspect_ratio = 1, label=:false)

    scatter!(p, [(xL1+mu_c*d_host) * R_HU_in_kpc], [0.0], 
                xlims=(-d_host*R_HU_in_kpc, d_host*R_HU_in_kpc),
                ylims=(-d_host*R_HU_in_kpc, d_host*R_HU_in_kpc),
                aspect_ratio = 1, label=:false)
    ...
    # Tidal radius 
    nbo = 500
    tabtx = [(d_host + rt * cos(2*pi/nbo * (i-0.5))) * R_HU_in_kpc for i=1:nbo]
    tabty = [rt * R_HU_in_kpc * sin(2*pi/nbo * (i-0.5)) for i=1:nbo]


    plot!(p, tabtx, tabty, 
                xlims=(-1.1*d_host*R_HU_in_kpc, 1.1*d_host*R_HU_in_kpc),
                ylims=(-1.1*d_host*R_HU_in_kpc, 1.1*d_host*R_HU_in_kpc),
                linestyle=:solid,
                color=:blue,
                aspect_ratio = 1, label=:false, frame=:box,
                size=(600, 600),
    ...
    scatter!(p, datax .* R_HU_in_kpc, datay .* R_HU_in_kpc)
...
namefile = path_data*"snapshots_"*srun_data*"/time_300.0.txt"


compute_tE(namefile)
plot_Lagrange_points(namefile)
